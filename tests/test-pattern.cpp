...
TEST_CASE("Long pattern") { 
  std::uint8_t test_arr[] = {0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0xFF, 0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x88, 0x99, 0x00, 0xFF, 0xAA, 0xBB}; 
  pattern<0xBB, ANY, 0xDD, 0xEE, ANY, 0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x88, 0x99, 0x00, 0xFF, 0xAA> p{};
  std::uintptr_t result = p.scan_match(test_arr, sizeof(test_arr));
  CHECK(result == reinterpret_cast<std::uintptr_t>(&*(test_arr + 1)));
}

TEST_CASE("Long wildcard pattern") { 
  std::uint8_t test_arr[] = {0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0xFF, 0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x88, 0x99, 0x00, 0xFF, 0xAA, 0xBB}; 
  pattern<0xBB, ANY, 0xDD, ANY, ANY, 0x00, ANY, 0x22, 0x33, ANY, 0x55, 0x66, 0x77, 0x88, ANY, 0x00, ANY, 0xAA> p{};
  std::uintptr_t result = p.scan_match(test_arr, sizeof(test_arr));
  CHECK(result == reinterpret_cast<std::uintptr_t>(&*(test_arr + 1)));
}
