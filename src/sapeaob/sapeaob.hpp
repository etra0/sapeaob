...
// Wrapper around the BSF instruction for x86 assembly. This one uses the x64 bits variant.
...
template <std::uint16_t... Pattern> struct pattern {
  ...
  // return the offset.
  // In the case there's nothing found, this function throws the `sapeaob::pattern_not_found` exception.
  template <class it> std::size_t scan_match_offset(it arr, std::size_t size);

  // Search for the pattern. If there's a match, returns the pointer of the match.
  // If there's nothing found, this function throws the `sapeaob::pattern_not_found` exception.
  template <class it> std::uintptr_t scan_match(it arr, std::size_t size);

  // Find all the matches in the array. If nothing is found, this function returns an empty vector.
  template <class it>
  std::vector<std::uintptr_t> find_all_matches(it arr, std::size_t size) noexcept;

private:
  template <class it>
  // Implementation of the `scan_match_offset`. This one uses offset since if there's a match,
  // we can still iterate from the first offset.
